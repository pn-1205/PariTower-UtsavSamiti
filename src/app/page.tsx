...
export default function DashboardPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 space-y-6">
      ...
      {/* MONEY BREAKDOWN & EXPENSE BREAKDOWN */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200">
          <div className="flex items-center justify-between pb-3 border-b border-gray-100">
            ...
            <Link href="/deposits" className="text-xs font-bold text-emerald-700 hover:underline">
              Ledger →
            </Link>
          </div>
          ...
          <div className="divide-y divide-gray-100 mt-2">
            <div className="py-3 flex items-center justify-between">
              <div>
                <p className="text-sm font-bold text-gray-800">From Regular Flats</p>
                <p className="text-xs text-gray-500">262 residential units</p>
              </div>
              <span className="text-base font-bold text-gray-900">
                {formatCurrency(receivedFromFlats)}
              </span>
            </div>

            <div className="py-3 flex items-center justify-between">
              <div>
                <p className="text-sm font-bold text-gray-800">From Other Contributors</p>
                <p className="text-xs text-gray-500">Guests, sponsors, and local vendors</p>
              </div>
              <span className="text-base font-bold text-gray-900">
                {formatCurrency(receivedFromOther)}
              </span>
            </div>

            <div className="py-3 flex items-center justify-between bg-emerald-50/50 px-3 rounded-xl mt-2">
              <span className="text-sm font-bold text-emerald-950">Total Money Received</span>
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200">
          <div className="flex items-center justify-between pb-3 border-b border-gray-100">
            ...
            <Link href="/expenses" className="text-xs font-bold text-rose-700 hover:underline">
              All Expenses →
            </Link>
          </div>
          ...
          <div className="space-y-3 mt-3">
            {expensesByCategory.length === 0 ? (
              ...
              expensesByCategory.slice(0, 5).map((cat: any) => {
                const catPercent = totalExpenses > 0 ? Math.round((cat.amount / totalExpenses) * 100) : 0;
                return (
                  <div key={cat.category} className="space-y-1">
                    <div className="flex justify-between text-xs font-semibold">
                      ...
                      <span className="text-gray-900">
                        ...
                        <span className="text-gray-400 font-normal">({catPercent}%)</span>
                      </span>
                    </div>
            ...
            <div className="py-2.5 flex items-center justify-between bg-rose-50/50 px-3 rounded-xl mt-3">
              <span className="text-sm font-bold text-rose-950">Total Expenses</span>
